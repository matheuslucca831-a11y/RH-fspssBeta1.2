...
def carregar_usuarios():
    if not os.path.exists("banco_usuarios.csv"):
        # cria arquivo vazio com as colunas corretas
        df = pd.DataFrame(columns=["email", "nome", "cargo", "matricula"])
        df.to_csv("banco_usuarios.csv", index=False)
        return []

    df = pd.read_csv("banco_usuarios.csv", dtype=str)
    return df.to_dict(orient="records")

# Inicializa db_usuarios ANTES de qualquer coisa
if "db_usuarios" not in st.session_state:
    st.session_state.db_usuarios = carregar_usuarios()  # ou o método que você usa
if len(st.session_state.db_usuarios) == 0:
    usuario_padrao = {
        "email": "0001",  # matrícula
        "nome": "Gestor Master",
        "cargo": "Gestor Máximo",
        "matricula": gerar_hash("admin123")  # senha
    }

    st.session_state.db_usuarios.append(usuario_padrao)
    salvar_csv("banco_usuarios.csv", st.session_state.db_usuarios)
